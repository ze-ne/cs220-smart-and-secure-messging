...
class ConversationUnitTests {
    ...
    fun send_validTextMessage() {
        ...
        verify(view).updateDisplayedMessages(TextMessage("Hi","1", sender.userId, reciever.userId, Math.toIntExact(date.time)))
    }

    @Test
    fun send_validImageMessage() {
        presenter.sendImageMessage(arrayOf())
        verify(view).updateDisplayedMessages(ImageMessage(ArgumentMatchers.any(ByteArray::class.java), "1", sender.userId, reciever.userId, Math.toIntExact(date.time)))
    }
    ...
    fun send_longTextMessage() {
        ...
        verify(view).updateDisplayedMessages(TextMessage("Hi, this is Zene. I'm sending this really long message for testing purposes only and not for any other reason","1", sender.userId, reciever.userId, Math.toIntExact(date.time)))
    }

    @Test
    fun send_nonAlphaTextMessage() {
        presenter.sendTextMessage("Hi --- just here...\"testing\" some th1ng5! c u l8r")
        verify(view).updateDisplayedMessages(TextMessage("Hi --- just here...\"testing\" some th1ng5! c u l8r","1", sender.userId, reciever.userId, Math.toIntExact(date.time)))
    }
